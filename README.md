...
| `olive-forget` | Erase every measurement in a time range and record the erasure as a disclosed gap: `olive-forget --from 2026-09-06T21:00 --to 2026-09-07T02:00 --reason "a guest"`. The window is then reported as **not monitored**, never as quiet -- it subtracts from the coverage figures, hatches in the calendar, and is listed in the report with the reason given (or "no reason given"). `--dry-run` prints what would go and changes nothing; without `--yes` it asks for the word `erase` first. Sessions and calibration history are kept, because they are lineage for the rows that remain. This is the only supported way to remove a window: editing the SQLite file by hand leaves a hole that reads as a quiet night. |
| `olive-report` | Render the accessible HTML report (distributions + day×hour calendar heatmap + quiet-hours summary). Optional `--csv` event export, `--violations-csv` / `--violations-html` for an honest quiet-hours report suitable for a neighbor/landlord/HOA submission, and `--pdf` / `--violations-pdf` for a tagged PDF/A-3a of either (needs the `pdf` extra; see [Standards Conformance](#standards-conformance)). A threshold-sensitivity section recounts events and loud minutes at ±3 and ±6 dB from the configured threshold, from data already stored; `--sensitivity off` omits it. Its wording has **not** yet had the acoustics-SME review its own human gate calls for. |
| `olive-bundle` | Assemble a tamper-evident evidence bundle -- `olive-bundle build --db olive.db --out bundle/` -- or check one: `olive-bundle verify bundle/`. The bundle holds a consistent SQLite snapshot, the report, both CSV exports, the quiet-hours HTML, the session ledger and the calibration history, plus `manifest.json` carrying the SHA-256 of every one of them and a plain-language block saying what a hash does and does not prove. `verify` gives three answers and only three: **intact**, **modified** (naming the file), or **unverifiable** -- the last is a real answer and never a polite form of the other two, so a bundle with no manifest is never reported as intact. The inventory is closed, so a file *added* to a bundle is a finding too. Passing the same `--created-at` twice makes two bundles of one database byte-identical. No signature is made or checked yet; a signature file found in a bundle yields **unverifiable**, not a shrug. |
...
| Code Quality | Applies — open gap recorded in [GAP-CQ-1](./docs/GAP-LEDGER.md#gap-cq-1--code-quality-python-floor-pre-commit-hook-wiring-src-layout-hatchling) (Python-floor divergence recorded in [ADR-0002](./docs/adr/0002-python-39-floor.md); the committed pre-commit hook set **is** enforced since 2026-09-05 — `make hooks` runs it over every tracked file, inside `make verify` and inside the required `verify` CI job, with the two hooks it deliberately does not run named in the `Makefile`; hatchling and `src/` layout still open) |
...
| Documentation | Applies — open gap recorded in [GAP-DOC-1](./docs/GAP-LEDGER.md#gap-doc-1--documentation-vendor-standards-as-a-pinned-submodule-finish-the-adr-migration) (`/STANDARDS` vendoring blocked on a portfolio-level tag prerequisite. The ADR migration is **done** as of 2026-09-05: the 25 decisions embedded in `docs/ROADMAP.md` are now numbered, append-only records under [`docs/adr/`](./docs/adr/), and the roadmap indexes them instead of restating them) |
